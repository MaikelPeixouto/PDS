...
class ApiService {
  ...
  async getClinic(id: string | number): Promise<any> {
  ...
  async getPublicClinicServices(clinicId: string): Promise<any[]> {
    const response = await this.request<{ services: any[] }>(`/clinics/${clinicId}/services`);
    return (response as any).services || response.data?.services || [];
  }
