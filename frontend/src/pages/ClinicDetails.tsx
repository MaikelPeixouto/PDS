...
const ClinicDetails = () => {
  ...
  const { data: services = [], isLoading: isLoadingServices } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return [];
      return api.getPublicClinicServices(id);
    },
    enabled: !!id,
  });
  ...
  const { data: existingReview } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        // Check if user has ANY review for this clinic (or specific appointment if selected)
        const review = await api.getUserReview(id, selectedAppointmentForReview || undefined);
        return review;
      } catch (error) {
        return null;
      }
  ...
  const handleSubmitReview = () => {
    ...
    const appointmentId = selectedAppointmentForReview || (completedAppointments.length > 0 ? completedAppointments[0].id : undefined);
    ...
    createReviewMutation.mutate({
      clinicId: id,
      appointmentId: appointmentId || undefined, // Send undefined/null instead of empty string
      rating: reviewRating,
            ...
            <Card className="p-6">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <div className="flex items-center gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star
                          key={star}
                          className={`h-5 w-5 ${star <= Math.round(clinic.rating || 0)
              ...
              {user && !existingReview && (
                <Card className="p-6 mb-6 bg-vet-primary/5">
                  ...
                  <div className="space-y-4">
                    {/* Appointment selection is now optional/removed for general reviews */}
                    {/* We can keep it if we want to link it, but user asked to review WITHOUT appointment */}
